...
# private aws_route ###
resource "aws_route" "private_route" {                                   #terraform aws route #(route edit chesi destination lo igw evvali only public)
  route_table_id            = aws_route_table.private.id       # go to search route table name what if you entered in route table then enter aws route
  destination_cidr_block    = "0.0.0.0/0"          # igw
  nat_gateway_id = aws_nat_gateway.NAT.id
}

# private aws_route ###
resource "aws_route" "database_route" {                                   #terraform aws route #(route edit chesi destination lo igw evvali only public)
  route_table_id            = aws_route_table.database.id       # go to search route table name what if you entered in route table then enter aws route
  destination_cidr_block    = "0.0.0.0/0"          # igw
  nat_gateway_id = aws_nat_gateway.NAT.id
}
